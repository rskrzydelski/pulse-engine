<script>
    ...
    import { calc_userWalletValue, calc_gasConsumption } from '../calc'
    ...
    onMount(async () => {
        ...
        const interval_2 = setInterval(async () => {
            const obj_pulsex = await graph_getTokenPrice(
                token_contracts.PLSX_TOKEN
            )
            const obj_phex = await graph_getTokenPrice(
                token_contracts.pHEX_TOKEN
            )
            const obj_inc = await graph_getTokenPrice(token_contracts.INC_TOKEN)

            $prices['PLSX'] = obj_pulsex.token_dollar_price
            $prices['PLS'] =
                (1 / Number(obj_pulsex.token_pls_price)) *
                Number(obj_pulsex.token_dollar_price)
            $prices['WPLS'] =
                (1 / Number(obj_pulsex.token_pls_price)) *
                Number(obj_pulsex.token_dollar_price)
        ...
        if (typeof dai_price === 'number' && dai_price > 0.0) {
            ...
            if ($debug)
                console.debug(
                    `🚨 can't fetch dai price, I assign $1.0 by default.`
                )
            $prices['DAI'] = 1.0
        }
        ...
        if (typeof usdc_price === 'number' && usdc_price > 0.0) {
            ...
            if ($debug)
                console.debug(
                    `🚨 can't fetch usdc price, I assign $1.0 by default.`
                )
            $prices['USDC'] = 1.0
        }
        ...
        if (typeof usdt_price === 'number' && usdt_price > 0.0) {
            ...
            if ($debug)
                console.debug(
                    `🚨 can't fetch usdt price, I assign $1.0 by default.`
                )
            $prices['USDT'] = 1.0
        }
        ...
        if (typeof eth_price === 'number' && eth_price > 0.0) {
            ...
            if ($debug)
                console.debug(
                    `🚨 can't fetch eth price, I assign NaN by default.`
                )
            $prices['ETH'] = NaN
            $prices['WETH'] = NaN
        }
    ...
    async function onSubmit() {
        ...
        const masterchef_data_promise =
            account_masterChefUserFarmsData(user_address) // farms data where user is participated
        ...
        const pools_receipts = await account_getRelevantReciepts(
            masterchef_data,
            user_txs
        )

        const prov_liquidities_promise = account_getUserProvidedLiquidity(
            masterchef_data,
            pools_receipts,
            user_address
        )
        ...
        const first_liquidity_timestamps = account_getFirstLiquidityTimestamp(
            masterchef_data,
            pools_receipts
        )
        ...
        user_data_update_timer = setInterval(async () => {
            ...
            const gas_pls_cost = calc_gasConsumption(pools_receipts)
            const first_liquidity_timestamps =
                account_getFirstLiquidityTimestamp(
                    masterchef_data,
                    pools_receipts
                )
        ...
        if ($debug)
            console.debug(
                `🚄 performance time: ${performance_t2 - performance_t1} ms`
            )
    }
...
<div class="dashboard">
    ...
    <div class="logo">
        <p class="pulse-engine {on_chain === false ? 'active' : ''}">
            Pulse Engine
        </p>
        <span class="ver {on_chain === false ? 'active' : ''}"
            >version 0.0.3</span
        >
    </div>
    ...
    <div class="info-box {on_chain === false ? 'active' : ''}">
        <div class="info">
            The app now supports the following pools: {Object.keys(
                lp_token_contracts
            ).map((pool) => pool.replace('_POOL', '') + ' ')}.
        </div>
        <div class="info">
            This is a beta version, notice that it can have bugs and poor
            performance.
        </div>
        <div class="info">I will refine this tool step by step.</div>
        <div class="info">
            I would appreciate your reporting any bugs, improvements, or new
            future ideas on my telegram channel
            <div>
    ...
    <form
        class="address-input-box {on_chain === true ? 'on-chain' : ''}"
        on:submit|preventDefault={async (e) => await onSubmit(e)}
    >
        <div>
            <input
                type="text"
        ...
        <div>
            <button
                ...
                aria-busy={on_chain}
                class="outline"
                disabled={data_collected}>look on chain 🔗</button
            >
        </div>
    </form>
